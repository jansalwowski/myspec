...
## Output Location
Write to `${aiDir}/anti-patterns.md` — append to the **project section only** (below the `<!-- myspec:framework-end -->` marker). The framework section between the markers is update-managed; modifying it violates the setup skill's rules and gets overwritten by `/myspec:update`.
