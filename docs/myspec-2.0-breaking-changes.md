...
### 7. Rewrite and rename the skill surface

**Problem.** 44 skills; descriptions total 10,787 characters (~2.7k tokens) loaded every session, with `doctor` alone at 642 characters against the repo's 500 rule. Families: 18 `feature-*` plus `features-status-audit`, 5 `memory-*` plus `memorize` and `memorify`, 3 `session-*`, 2 `idea-*`, 13 other. Names are inconsistent where a rename is already landing (`ai-setup-audit` → `doctor`).
...
- `docs-sanitize` is routed to by nothing. Two of its three jobs were already covered — dead references by `doctor` surface C, and dangling sessions by `session-clean`. The other two were not, and retiring it moves them: the `${aiDir}` kebab-case naming sweep becomes a `doctor` surface C bullet, and archiving a live log whose `status:` is already terminal becomes a `session-clean` Step 4 row (its liveness gate previously skipped exactly that file).
...
- Descriptions are rewritten, not truncated: each keeps its `Use when…` trigger condition (`rules/skill-optimization.md` requires that opening, and `skill-verify` enforces it with `/^Use when/`), drops keywords derivable from the skill's own name, and keeps the "Do NOT" clauses that name a genuinely confusable neighbour or guard a precondition. **43 model-invocable skills, 10,137 chars, none over 350** (`doctor` is exactly 350, down from 642).
...
**Why the collapse was reverted.** It removed 13 entry points from slash-command autocomplete (memory −6, session −2, idea −1, mockup-review −1, scenario + seed-data −2, docs-sanitize −1). Autocomplete completes *skill names*; an argument is free text it cannot see, and skill frontmatter has no `argument-hint` — that is a custom-slash-command feature, not a skill one. So `/myspec:memo…` stopped offering seven completions and offered one, with the six modes discoverable only by reading the router's Modes table. The maintainer uses that completion heavily. The diet alone captures −6% (10,787 → 10,137, both measured over model-invocable skills); the collapse reached 6,826 chars, so it would have added roughly a further 3,300 chars, about 830 tokens a session. That was judged the wrong trade against daily discoverability. The diet's real win is not the total — it is the per-description cap: the worst offender drops 642 → 350 and nothing exceeds 350, so no single skill can crowd out a consumer's own.

Two secondary costs the collapse also carried, both avoided by the revert: `memory-lookup` is `allowed-tools: [Read, Grep, Glob]` and `memory-preflight` is `[Read, Grep, Glob, Bash]` — neither can write, and a merged skill that also writes cannot keep that restriction; it would have degraded to prose. And the retirement stubs are live skills for one minor cycle, so the collapsed surface would have been 34 files until they were removed.
...
**Migration.** Plugins have no alias or redirect mechanism; an old name simply fails. Three stubs — `features-status-audit`, `worktree-cleanup`, `docs-sanitize` — each name their replacement and stop; removed one minor cycle after 2.0. Each carries `disable-model-invocation: true`, so the stub stays reachable by slash command and autocomplete but its description never enters the system prompt: 46 skill directories ship, 43 descriptions load. That is also why their descriptions are plain labels rather than `Use when…` triggers (`rules/skill-optimization.md` manual-only exception). `rules/workflow.md`, README, `examples/`, and the affected `Integration` sections update with the renames.
