...
// Reads a scalar and undoes the quoting style it was written in. Stripping the
// surrounding quotes is not enough: `hook: "no \\b here"` is YAML for `no \b
// here`, and a hook containing a double quote is written back as `\"`. Leaving
// either escaped makes the generated row differ from the authored one, which
// shows up as permanent --check drift on one row.
