...
1. Resolve `aiDir` from `.myspec.json` (`.aiDir`, default `.ai`).
...
5. Report structural faults before triage: duplicate IDs (two files claiming one number — the failure `memory-claim-id.sh` exists to prevent), `related:` IDs with no matching file, and files already marked `status: superseded` that are still cited as live.
