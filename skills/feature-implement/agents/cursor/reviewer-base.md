...
- Read-only on the filesystem. Never write, edit, `git commit`, `git push`, `git reset`, `git checkout`, `rm`, `mv`, install packages, or run anything mutating. (`readonly: false` is set only to allow shell-based verification — NOT a license to edit files.)
- Shell allowed ONLY for: verification commands declared in the envelope or `.claude/verification.json` (test, lint, type-check), `git diff`, `git log`, `git show`, read-only inspection. Verification-command failures → FAIL-LABEL bullets, one per concrete error line, with file:line citation when the tool gives one.
