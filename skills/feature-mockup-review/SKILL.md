...
description: "Use when reviewing mockups in ${aiDir}/features/{feature}/mockups/ for UX issues, scope creep, loose ends (dead buttons/links), missing states, production-flow fidelity, accessibility, or project hard-guard violations. Keywords: review mockup, critique mockup, mockup ux check, mockup audit, mockup loose ends. Do NOT use for spec.md review (use feature-spec-review), tech-spec.md review (use feature-tech-spec-review), production-code review (use code-review), or generating new mockup files (use feature-mockup)."
...
```dot
digraph {
    "Resolve scope" -> "Silent recon";
    "Silent recon" -> "Dossier (siblings + scope confirm)";
    "Dossier (siblings + scope confirm)" -> "Run 5 review groups";
    "Run 5 review groups" -> "Findings table";
    "Findings table" -> "Group fixes (deterministic batch + judgement)";
    "Group fixes (deterministic batch + judgement)" -> "Present deterministic batch — bulk approval";
    "Present deterministic batch — bulk approval" -> "Present judgement fixes — per item";
    "Present judgement fixes — per item" -> "Apply approved fixes";
    "Apply approved fixes" -> "Re-verify per file";
    "Re-verify per file" -> "Wrap up + handoff";
}
```
...
Each catalog entry is a 1-line pattern + 1-line detection cue. Reading the index first lets you skip catalogs that don't apply.
