---
name: "feature-mockup"
description: "Use when building UI mockups for a feature under ${aiDir}/features/{feature}/mockups/ — exploring a flow, validating a spec visually, prototyping screens, edit modals, or admin surfaces before tech design. Keywords: mockup, mock up, prototype screen, wireframe, visualize spec, UX exploration. Uses the project's design-system defaults at all times; never edits production code or schema. Do NOT use for production UI code, schema/migration changes, new design-system components, or distinctive-aesthetic landing pages (use frontend-design for those)."
...
```dot
digraph {
    "Resolve feature" -> "Silent recon (incl. mockup-design.md + reference pick)";
    "Silent recon (incl. mockup-design.md + reference pick)" -> "AskUserQuestion: dossier (≤4 q's)";
    "AskUserQuestion: dossier (≤4 q's)" -> "Propose inventory (variants? markers)";
    "Propose inventory (variants? markers)" -> "User confirms" [label="numbered list, one-line rationale each"];
    "User confirms" -> "Build loop" [label="per file"];
    "Build loop" -> "Write file";
    "Write file" -> "verify command";
    "verify command" -> "compile-check";
    "compile-check" -> "Build loop" [label="next file"];
    "compile-check" -> "Iterate" [label="all files done"];
    "Iterate" -> "Edit + re-verify" [label="feedback"];
    "Edit + re-verify" -> "Iterate";
    "Iterate" -> "Wrap up" [label="done / topic shift"];
    "Wrap up" -> "Rule extraction (if ≥2× repeats)";
    "Rule extraction (if ≥2× repeats)" -> "Commit prompt";
    "Commit prompt" -> "Print handoff list";
}
```
...
2. **Silent recon** — read these without asking the user. **Data-model/spec mismatches are the #1 source of rebuild rounds — never invent enum values or field names from a verbal description.** If the spec is ambiguous about a model shape, ask the user before guessing.
   ...
   - **Component-library surface** — the library's export index / component list (named in `mockup-design.md`), so every pick is a real component.
   ...
   - **Catalog skim** — if the planned mockups include form inputs, tabular data views, or admin surfaces, skim the matching catalog at [../feature-mockup-review/references/](../feature-mockup-review/references/): `forms.md`, `tables.md`, `admin-dashboard.md`. Always skim `core.md` and `accessibility.md`. The review skill will check against these — building to them prevents the round-trip.
